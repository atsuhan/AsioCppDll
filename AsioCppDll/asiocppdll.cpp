...
//MicSetting from Unity
int SampleRate = 44100;	//初期値
int SamplingLength = 2048;	//初期値
extern "C" {
	void SetMicSetting(int sampleRate, int samplingLength){
...
{	// the actual processing callback.
	...
	for (int i = 0; i < asioDriverInfo.inputBuffers + asioDriverInfo.outputBuffers; i++)
	{
		...
		{
			int *curbuff = (int*)asioDriverInfo.bufferInfos[i].buffers[index];
			...
			case ASIOSTInt32LSB:
				...
				for (int i = buffSize / 2; i < buffSize; i++) {
					curbuff[i] = 100000000;
				}
		...
		//Input
			...
			if (0 <= asioDriverInfo.bufferInfos[i].channelNum && asioDriverInfo.bufferInfos[i].channelNum < 4){
				if (RecStart == true) {
					if (ProcessedSamples < SamplingLength) {
						if (asioDriverInfo.bufferInfos[i].channelNum == 0) {
							memcpy(&MemMic0[ProcessedSamples], asioDriverInfo.bufferInfos[i].buffers[index], buffSize * 4);
						}
						if (asioDriverInfo.bufferInfos[i].channelNum == 1) {
							memcpy(&MemMic1[ProcessedSamples], asioDriverInfo.bufferInfos[i].buffers[index], buffSize * 4);
						}
						if (asioDriverInfo.bufferInfos[i].channelNum == 2) {
							memcpy(&MemMic2[ProcessedSamples], asioDriverInfo.bufferInfos[i].buffers[index], buffSize * 4);
						}
						if (asioDriverInfo.bufferInfos[i].channelNum == 3) {
							memcpy(&MemMic3[ProcessedSamples], asioDriverInfo.bufferInfos[i].buffers[index], buffSize * 4);
						}
					}
					else {
						RecStart = false;
...
{	// create buffers for all inputs and outputs of the card with the 
	...
	ASIOChannelInfo *ch_info = asioDriverInfo->channelInfos;
	...
	if (asioDriverInfo->inputChannels > kMaxInputChannels)
		...
		asioDriverInfo->inputBuffers = 4;
	}

	for (i = 0; i < asioDriverInfo->inputBuffers; i++, info++, ch_info++)
	{
		info->isInput = ASIOTrue;
		info->channelNum = i;
		info->buffers[0] = info->buffers[1] = 0;
		//		ch_info->type = ASIOSTInt32LSB;
		//		ch_info->type = ASIOSTFloat32LSB;
	}
	...
	for (i = 0; i < asioDriverInfo->outputBuffers; i++, info++, ch_info++)
	{
	...
	if (result == ASE_OK)
	{
		...
		for (i = 0; i < asioDriverInfo->inputBuffers + asioDriverInfo->outputBuffers; i++)
		{
			...
			asioDriverInfo->channelInfos[i].type = ASIOSTFloat32LSB;
		}
...
extern "C" {
	...
	void MainStart() {
		...
		MemMic0 = new int[SamplingLength];
		MemMic1 = new int[SamplingLength];
		MemMic2 = new int[SamplingLength];
		MemMic3 = new int[SamplingLength];
	...
	void MainCalled(int* tempMem0, int* tempMem1, int* tempMem2, int* tempMem3) {
		
		ProcessedSamples = 0;
		RecStart = true;
		while (RecStart == true)
		{
			Sleep(10);	// goto sleep for 10 milliseconds
		}

		//書き出し用
		char *filename0 = "Mic0.bin";
		char *filename1 = "Mic1.bin";
		char *filename2 = "Mic2.bin";
		char *filename3 = "Mic3.bin";
		fp[0] = fopen(filename0, "wb");
		fp[1] = fopen(filename1, "wb");
		fp[2] = fopen(filename2, "wb");
		fp[3] = fopen(filename3, "wb");
		fwrite(MemMic0, SamplingLength, 4, fp[0]);
		fwrite(MemMic1, SamplingLength, 4, fp[1]);
		fwrite(MemMic2, SamplingLength, 4, fp[2]);
		fwrite(MemMic3, SamplingLength, 4, fp[3]);
		fclose(fp[0]);
		fclose(fp[1]);
		fclose(fp[2]);
		fclose(fp[3]);
		//Memory
		for (int a = 0; a < SamplingLength; a++)
		{
			tempMem0[a] = MemMic0[a];
			tempMem1[a] = MemMic1[a];
			tempMem2[a] = MemMic2[a];
			tempMem3[a] = MemMic3[a];
		}
	}
